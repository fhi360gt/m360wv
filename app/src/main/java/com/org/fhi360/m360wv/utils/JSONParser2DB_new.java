 ...
 */
public class JSONParser2DB_new {
    ...
    public JSONParser2DB_new(String jsonString) {
    ...
    public void createAndSaveDB(Context ctx) {
        Conexion cnAnalytics = new Conexion(ctx, STATICS_ROOT + File.separator + "analytics.db",null,4);
        SQLiteDatabase dbAnalytics = cnAnalytics.getWritableDatabase();
        String query;
        //Creacion de tablas
        for (int i = 0; i< tables.size(); i++) {
            //query = "CREATE TABLE IF NOT EXISTS " + tables.get(i) + " ( ";
            int position = columns.get(i).indexOf("maddress");
            //Log.d("CREATE", Integer.toString( position));
...
//                }
                ...
                query = "Insert into tblresults (source_form, macadr, var_form, result) values (";
                query= query+"'"+tables.get(i)+"','"+values.get(i).get(position)+"','"+columns.get(i).get(j)+"','"+values.get(i).get(j)+"'";
                query = query + ");";
                Log.d("CREATE",query);
