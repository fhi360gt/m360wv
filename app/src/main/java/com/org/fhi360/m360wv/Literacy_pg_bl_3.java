...
//public class Reach_pg_sm_1 extends Activity implements View.OnClickListener{
    ...
    private void load_indicadores () {
        ...
        String sql = "SELECT  indicator FROM (\n" +
                "SELECT \"No lesson plan: Have a prepared lesson plan that is written up in the agreed upon structure (either MoE or Teacher Training) that is accessible during the class\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q70\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Inadequate preparation for the lesson: Focus on sequencing of lesson stages and materials required, as articulated in the lesson plan\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q71\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Gaps in content knowledge of LB topics: Observer should provide coaching/guidance to address and knowledge gaps based on relevant teacher training topics\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q77\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Student Learning Materials – textbooks, notebooks, pens/paper, books: for textbooks – raise issue with school director or district education office; pens/paper,books – recommend strategies for engaging parents to ensure children have required materials\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q73\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Teaching Materials – Chalk/chalkboard, posters, charts, props, books: refer back to training on locally developed teaching and learning materials and strategies the teacher can employ in the classroom\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q74\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Misalignment of teaching strategy for topic:  ask teacher to reflect upon the component of the lesson with misaligned strategy/approach to topic. Encourage teacher to explore other methods of delivering that topic, providing suggestions as appropriate\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q79\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Poor classroom management:  established classroom rules/code of conduct visible and know by students; student awareness of consequences; consistent enforcement of classroom rules/code of conduct; ensure that lessons are keep child engaged (provide positive examples to enhance engagement)\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q81\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Wasted time (Periods of zero time on task / complete non-engagement): Question teaching and learning strategies that are not effectively engaging children – provide positive examples to enhance engagement \" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q82\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Unplanned multigrade classes (due to teacher absenteeism): Check with teacher if this is regular; if so work with teacher to identify strategies to overcome\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q76\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Large class sizes:  discuss reason behind large class size – a few options – (1) high enrollment due to high levels of repetition or late enrollment; (2) teacher absenteeism that leads to multigrade classes (Discuss with teacher if there are any ideas for how to overcome these issues) Issue should be raised with school director or district education office\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q75\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Desks/Chairs/Floor Mats:  issue should be raised with school director or district education office\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Lesson\" AND var_form = \"q72\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
