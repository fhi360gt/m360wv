...
//public class Reach_pg_sm_1 extends Activity implements View.OnClickListener{
    ...
    private void load_indicadores () {
        ...
        String sql = "SELECT  indicator FROM (\n" +
                "SELECT \"Gaps in content knowledge of Reading Camp activities:  Observer should provide coaching/guidance, referring to the main points from the training manual related to the area the facilitator is demonstrating gaps in knowledge\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q82\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Wasted time (Periods of zero time on task / complete non-engagement of children): Ask the facilitator about the times when children weren't engaged and why they thought that happened. Move discussion to identifying positive examples to enhance engagement of children. Refer to the checklist for teaching and learning strategies\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q84\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Many children in attendance (greater than 35 children):  discuss reasons so many children attending and engage the facilitator to identify solutions including adding another session each week, splitting off ECCD children to a separate group, and get the SMC/school director support\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q80\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Teaching/learning Materials – Chalk/chalkboard, posters,  journals, etc: refer back to training on locally developed teaching and learning materials and strategies the facilitator can employ to replenish or access materials for the reading club sessions\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q70\" AND CAST(result AS INTEGER) = 0 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Teaching/learning Materials – Chalk/chalkboard, posters,  journals, etc: refer back to training on locally developed teaching and learning materials and strategies the facilitator can employ to replenish or access materials for the reading club sessions\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q76\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Teaching/learning Materials – Chalk/chalkboard, posters,  journals, etc: refer back to training on locally developed teaching and learning materials and strategies the facilitator can employ to replenish or access materials for the reading club sessions\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q77\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Teaching/learning Materials – Chalk/chalkboard, posters,  journals, etc: refer back to training on locally developed teaching and learning materials and strategies the facilitator can employ to replenish or access materials for the reading club sessions\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q78\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Inadequate preparation for the reading club session: Focus on sequencing of session stages, timing, connecting theme around topic and core literacy skills and materials required\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q61\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Inadequate preparation for the reading club session: Focus on sequencing of session stages, timing, connecting theme around topic and core literacy skills and materials required\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q61\" AND CAST(result AS INTEGER) = 2 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Access to books –  Ask an open question about the lack of access to books in the session. Issues could be no presence of the book bank, books are damaged, facilitator not enabling access to available titles or books are not usable or children have read all the titles. Recommend strategies based upon the problem\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q75\" AND CAST(result AS INTEGER) = 1 "+  stringFilter + "\n" +
                "UNION\n" +
                "SELECT \"Suggestions typed by observer are displayed here\" AS indicator FROM  tblresults WHERE source_form =\"WV_Boost_Camp\" AND var_form = \"q58\"  "+  stringFilter + "\n" +
